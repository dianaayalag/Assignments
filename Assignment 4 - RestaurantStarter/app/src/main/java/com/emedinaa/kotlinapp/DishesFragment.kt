...
import com.emedinaa.kotlinapp.storage.remote.DishesRemoteDatasource
...
class DishesFragment : Fragment() {
    ...
    private val dishesRepository: DishesRepository by lazy {
        DishesRemoteDatasource(ApiClient())
    }

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        category = arguments?.getParcelable("CATEGORY") as? Category
        token = (requireActivity()).intent.extras?.getParcelable<Session>("SESSION")?.token ?: ""
    }
    ...
    private fun fetchDishes() {
        ...
        lifecycleScope.launch {
            when (val result =
                if (categoryId.isEmpty()) dishesRepository.getAll(token) else dishesRepository.getAll(
                    token,
                    categoryId
                )) {
